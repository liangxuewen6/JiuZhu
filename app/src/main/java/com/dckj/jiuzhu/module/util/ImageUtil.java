...
public class ImageUtil {
	...
	public static Bitmap getBitmap(String url, int desiredWidth, int desiredHeight) {
		...
		try {
			...
			e.printStackTrace();
			Log.d(ImageUtil.class.toString(), "" + e.getMessage());
		} finally {
			try {
				if (is != null) {
					is.close();
	...
	private static boolean checkBitmap(Bitmap bitmap) {
		if (bitmap == null) {
			Log.e(ImageUtil.class.toString(), "原图Bitmap为空了");
			return false;
		}

		if (bitmap.getWidth() <= 0 || bitmap.getHeight() <= 0) {
			Log.e(ImageUtil.class.toString(), "原图Bitmap大小为0");
			return false;
		}
		return true;
	}
	...
	private static boolean checkSize(int desiredWidth, int desiredHeight) {
		if (desiredWidth <= 0 || desiredHeight <= 0) {
			Log.e(ImageUtil.class.toString(), "请求Bitmap的宽高参数必须大于0");
			return false;
		}
		return true;
	}
		...
		if (bitmap != null) {
			try {
				if (!bitmap.isRecycled()) {
					Log.d(ImageUtil.class.toString(),
	...
	public static void releaseBitmapList(List<Bitmap> bitmaps) {
		if (bitmaps != null) {
			try {
				for (Bitmap bitmap : bitmaps) {
					if (bitmap != null && !bitmap.isRecycled()) {
						Log.d(ImageUtil.class.toString(),"Bitmap释放" + bitmap.toString());
						bitmap.recycle();
					}
				}
			} catch (Exception e) {
